...
export default function Specificall({ propId, onIdchange, propStatus }) {
  useEffect(() => {
...

    {
      propStatus == "for treatment"
        ? setAlertDataTIme({ ...alertDataTime, flag: true })
        : setAlertDataTIme({ ...alertDataTime, flag: false });
    }
  }, [propId]);
  const [data, setdata] = useState(null);
  const [PreviousAlert, setPreviousAlert] = useState({
    falseAlert: [],
    trueAlert: [],
  });
  const [PreviousAlertShow, setPreviousAlertShow] = useState({
    falseAlert: false,
    trueAlert: false,
  });
  const [modalVisible, setModalVisible] = useState(false);
  const [moreDetails, setmoreDetails] = useState(false);
  const [dataSpecificAlert, setdataSpecificAlert] = useState({
    flag: false,
    data: " ",
  });
  ...
  async function updateAlert(IdAlert, msgState) {
    try {
      ...
      const response = await axios.post(`alerts/treated/`, {
        id: IdAlert,
        status: msgState,
        userId: token._id,
        duration: alertDataTime.time,
        summary: alertData,
      });
      const result = response.data;
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      {data ? (
        <View>
          ...
          <Pressable style={styles.helpButton}>
            <Text style={styles.helpButtonText}>
              {data.alert.distressDescription}
            </Text>
          </Pressable>
          <View style={styles.dateTime}>
            <Text style={styles.dateText}>
              Date: {data.alert.date.split("T")[0]}
            </Text>
            <Text style={styles.timeText}>
              Time: {data.alert.date.split("T")[1].split(".")[0]}
            </Text>
          </View>
          <Pressable style={styles.audioButton}>
            <Ionicons name="play" size={24} color="white" />
            <Text style={styles.audioText}>
              Part :{data.alert.audio ? data.alert.audio : "Does not exist"}
            </Text>
          </Pressable>
          <Pressable
            onPress={() => {
          ...
          <View style={styles.menu}>
            <View style={styles.row}>
              ...
              <Pressable
                style={styles.column}
                onPress={() =>
                  setdataSpecificAlert({
                    ...dataSpecificAlert,
                    flag: !dataSpecificAlert.flag,
                    data: "Sensor",
                  })
                }
              >
                <MaterialIcons name="sensors" size={"200%"} color="white" />
                <Text style={{ color: "white", fontSize: " 150%" }}>
                  Sensor
                </Text>
              </Pressable>
            </View>
          ...
          <View style={styles.footer}>
            {propStatus == "for treatment" && (
              <Pressable
                ...
                onPress={() => {
                  alertData
                    ? (updateAlert(data.alert._id, "treated"),
                      onIdchange(""),
                      setAlertDataTIme({ ...alertDataTime, flag: false }),
                      setAlertData({}))
                    : alert("you need to make summry");
                }}
              >
        ...
        <Text style={styles.helpButtonText}>the server crashed</Text>
      )}
      {dataSpecificAlert.flag && (
        <View
          style={
            dataSpecificAlert.data != "Ticketing"
              ? styles.dataSpecificAlert
              : {
                  right: "165%",
                  bottom: "20%",
                  zIndex: 5,
        ...
        >
          {dataSpecificAlert.data == "Sensor" ? (
            ...
            >
              ...
              <Text style={styles.dataSpecificAlertText}>
                {" "}
                Adress:{data.alert.location.street}
                {"\n"}
          ...
          ) : dataSpecificAlert.data == "Evidence" ? (
            <UploadFiles />
          ) : dataSpecificAlert.data == "Trigger" ? (
            <Text style={styles.dataSpecificAlertText}>Type: Message</Text>
          ) : dataSpecificAlert.data == "Instructions" ? (
            <Text style={styles.dataSpecificAlertText}>
              no further instruction
            </Text>
          ) : dataSpecificAlert.data == "Ticketing" ? (
            <SOSAlertForm
              onDataAlert={{ handleSetAlertData }}
      ...
      {moreDetails && (
        <View style={styles.more}>
          ...
          >
            <Text style={{ color: "white", fontSize: "150%" }}>
              Previous alerts: {PreviousAlert.trueAlert.length}
            </Text>
          </Pressable>
          <Pressable
            onPress={() =>
          ...
          >
            <Text style={{ color: "white", fontSize: "150%" }}>
              Previous false alerts: {PreviousAlert.falseAlert.length}
            </Text>
          </Pressable>
